    ...
    for inp in node_inputs:
        if inp == "USER_QUERY":
            ...
            if isinstance(upstream, AgentResult):
                ...
                if upstream.agent_name == "critic":
                    target_nid = graph_nodes[inp].get("metadata", {}).get("target")
                    if not target_nid:
                        for c_inp in graph_nodes[inp].get("inputs", []):
                            if c_inp.startswith("n:") and c_inp != "USER_QUERY" and c_inp in graph_nodes:
                                target_nid = c_inp
                                break
                    if target_nid and target_nid in graph_nodes:
                        target_upstream = graph_nodes[target_nid].get("result")
                        if isinstance(target_upstream, AgentResult):
                            out.append({"id": target_nid, "kind": "upstream",
                                        "skill": target_upstream.agent_name, "output": target_upstream.output})
            else:
...
def parse_skill_json(text: str) -> dict:
    ...
    def sanitize_json_string(s: str) -> str:
        # Find double-quoted string literals and replace raw newlines within them
        pattern = re.compile(r'"(?:[^"\\]|\\.)*"')
        return pattern.sub(lambda m: m.group(0).replace('\n', '\\n').replace('\r', '\\r'), s)

    t_sanitized = sanitize_json_string(t)
    try:
        return json.loads(t_sanitized)
    except json.JSONDecodeError:
        start, end = t_sanitized.find("{"), t_sanitized.rfind("}")
        if start >= 0 and end > start:
            try:
                return json.loads(t_sanitized[start:end + 1])
            except json.JSONDecodeError:
                pass
    return {}
